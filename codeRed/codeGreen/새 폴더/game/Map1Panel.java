...
public class Map1Panel extends JPanel implements Runnable {
    ...
    public void update() {
        switch (characterType) {
            case 0 -> {
                playerU.update();
                muzan.setTargetCoordinates(playerU.getX(), playerU.getY());
                muzan.setEntityToFollow(playerU);
                muzan.followCoordinates();
            }
            case 1 -> {
                playerY.update();
                muzan.setTargetCoordinates(playerY.getX(), playerY.getY());
                muzan.setEntityToFollow(playerY);
                muzan.followCoordinates();
            }
            case 2 -> {
                playerM.update();
                muzan.setTargetCoordinates(playerM.getX(), playerM.getY());
                muzan.setEntityToFollow(playerM);
                muzan.followCoordinates();
            }
        }
    }
